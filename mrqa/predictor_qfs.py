...
from mrqa.prepro_utils import preprocess_text, encode_ids, encode_pieces, printable_text, SPIECE_UNDERLINE
...
_PrelimPrediction_qfs = collections.namedtuple(  # pylint: disable=invalid-name
    "PrelimPrediction_qfs",
    ["feature_index", "start_index", "end_index",
    "start_log_prob", "end_log_prob", "ans_probs"])
_NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
    "NbestPrediction", ["text", "start_log_prob", "end_log_prob"])
_NbestPrediction_qfs = collections.namedtuple(  # pylint: disable=invalid-name
    "NbestPrediction_qfs", ["text", "start_log_prob", "end_log_prob", "feature_index"])
...
def get_predictions_qfs(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    prelim_predictions = []
    word_to_ans_prob_final = {}

    # keep track of the minimum score of null start+end of position 0
    score_null = 1000000  # large and positive
    
    for (feature_index, feature) in enumerate(features):   # one example might has been converted to multiple segments, thus have multiple features
      ...
      # we need to map the origital tok index to its word index
      ...
      # (sudan) we map the ans_prob to it's original words
      para_seg = np.array(feature.segment_ids)==SEG_ID_P
      para_ids = (feature.input_ids*para_seg).tolist()
      ans_probs = (result.ans_probs*para_seg).tolist()
      para_len = sum(para_seg) # the number of True
      para_ids = para_ids[: para_len]
      ans_probs = ans_probs[:para_len]

      sp_model = spm.SentencePieceProcessor()
      sp_model.Load(FLAGS.spiece_model_file)

      para_pieces = sp_model.IdToPiece(para_ids) # ['▁parents', '▁often', '▁use', '▁videos', '▁games', '▁as', '▁a', '▁way', '▁to', '▁keep', '▁their', '▁children', '▁entertained', '▁while', '▁they', '▁are', '▁busy', '▁mind', 'ing', '▁their', '▁own', '▁business', '▁', '.', '▁this', '▁means', '▁that', '▁there', '▁is', '▁no', '▁real', '▁parenting', '▁supervision', '▁on', '▁the', '▁games', '▁', '.', '<sep>', '▁parenting', '▁consent', '▁', '-', '▁is', '▁parenting', '▁consent', '▁insufficient', '▁for', '▁the', '▁purpose', '▁of', '▁filtering', '▁violent', '▁games', '▁from', '▁their', '▁children', '▁', '?', '▁can', '▁increased', '▁censorship', '<sep>', '<cls>']
      para_words = sp_model.DecodePieces(para_pieces).split()

      cur_word = ''
      cur_word_ans_prob = 0.0
      word_to_ans_prob = []
      for (item_index, item) in enumerate(para_pieces):
        if item.startswith(SPIECE_UNDERLINE):
          if cur_word == '':
            cur_word = item.replace(SPIECE_UNDERLINE, '')
            cur_word_ans_prob = ans_probs[item_index]
            continue
          print("the current word is the {}, the word is {} and the word ans prob is {}".format(item_index, cur_word, cur_word_ans_prob))
          word_to_ans_prob.append(cur_word_ans_prob)
          cur_word = item.replace(SPIECE_UNDERLINE, '')
          cur_word_ans_prob = ans_probs[item_index]
        else:
          cur_word += item
      ...
      for i,j in zip(para_words, word_to_ans_prob):
        print("{}:{}".format(i,j))

      # assert len(example.paragraph_text.split()) == len(word_to_ans_prob)
      word_to_ans_prob_final[feature_index] = word_to_ans_prob
 
      # print("hello world")
      # print(len(word_to_ans_prob))
      # print(word_to_ans_prob)
      # print(len(para_words))
      # print(para_words)

      # print("debug =====sudan")
      # print(feature.input_ids)
      # print(result.ans_probs)
      # print(feature.p_mask)
      # print(feature.segment_ids)
      # print(example.paragraph_text)
      # print(np.count_nonzero(feature.input_ids))
      # print(np.count_nonzero(result.ans_probs))
      # print(np.count_nonzero(feature.p_mask))
    ...
    if not nbest:
      nbest.append(
          _NbestPrediction_qfs(text="", start_log_prob=-1e6,
          end_log_prob=-1e6, feature_index=0))
    ...
    for i, entry in enumerate(nbest):
      ...
      if not best_non_null_entry:
        ...
        best_non_null_idx = i
        best_feature_index = entry.feature_index
    
    print("the answer is {}".format(best_non_null_entry.text))
...
def mrqa_predictor_qfs(FLAGS, predict_fn, data):
  ...
  for num, eval_feature in enumerate(eval_features_inp):
    ...
    # if num > 2:
    #   break
    print("the {}th examples".format(num))
    ...
    print("back to softmax prob===")
    start_probs = ([float(math.exp(x)) for x in result["start_log_probs"].flat])
    end_probs = ([float(math.exp(x)) for x in result["end_log_probs"][:,0,:].flat])

    start_log_probs = ([float(x) for x in result["start_log_probs"].flat])
    end_log_probs = ([float(x) for x in result["end_log_probs"][:,0,:].flat])

    result['start_probs'] = start_probs
    result['end_probs'] = end_probs
    ans_probs = []
    # temporaly use the squared
    ans_prob = 0.0
    ...
    # print("The sum of the start_prob is %f " % sum(start_probs))
    # print("The sum of end prob is %f " % sum(end_probs))

    # print(len(result['start_log_probs']))
    # print(len(result["end_log_probs"]))

    # print(result['start_log_probs'])
    # print(result["end_log_probs"])
    # import numpy
    # print(numpy.shape(result['start_log_probs'])) # （1，521）
    # print(numpy.shape(result["end_log_probs"])) # (1,5,512)

    # print(result['end_log_probs'][0,0,:])
    # print(result['end_log_probs'][0,1,:])
