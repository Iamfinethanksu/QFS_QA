...
def get_predictions_qfs(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    for (feature_index, feature) in enumerate(features):   # one example might has been converted to multiple segments, thus have multiple features
      ...
      print(np.count_nonzero(feature.segment_ids))
    ...
    for pred in prelim_predictions:
      ...
      print("hello world")
      print(tok_start_to_orig_index)
      print(tok_end_to_orig_index)
      print(len(tok_start_to_orig_index))
      print(len(tok_end_to_orig_index))
      ...
      words = []
      for start,end in zip(tok_start_to_orig_index,tok_end_to_orig_index):
        word = paragraph_text[start: end + 1].strip()
        words.append(word)
...
def mrqa_predictor(FLAGS, predict_fn, data):
  ...
  for num, eval_feature in enumerate(eval_features_inp):
    ...
    # print(eval_feature['input_mask'])
    # print(eval_feature['segment_ids'])
    # print(eval_feature['p_mask'])
    ...
    cur_results.append(
      RawResult_qfs(
          ...
          cls_logits=cls_logits,
          ans_probs=ans_probs))
    # restuls_to_save['unique_id'] = 
    # results_to_save_list.append(restuls_to_save)
